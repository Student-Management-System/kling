...
export function copy<T>(target: T): T {
	...
	if (typeof target === "object") {
		if (typeof (target as any)[(Symbol as any).iterator] === "function") {
			const cp = [] as any[];
			...
			if (targetKeys.length > 0) {
				for (const key of targetKeys) {
					(cp as any)[key] = copy((target as any)[key]);
				}
			}
			return cp as T;
		}
 ...
 */
export function convertToEntityNoRelations<T extends BaseEntity>(
	target: new () => T,
	dto: unknown
): T {
